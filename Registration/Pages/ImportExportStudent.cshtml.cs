...
{
    public class ImportExportStudentModel : PageModel
    {
        ...
        public ImportExportStudentModel(IConfiguration config, IHostingEnvironment hostingEnvironment, PrincetonhiveContext context)
        {
        ...
        public async Task<IActionResult> OnPostAsync(string savemore)
        {
            ...
            if (savemore == "Export")
            {
                var studentlist = (from a in _context.TblStudentRegistration
                                   select new TblStudentRegistration { FirstName = a.FirstName, LastName = a.LastName, DisplayName = a.DisplayName, Gender = a.Gender, Dob = a.Dob, Address = a.Address, Username = a.Username,SchoolName=a.SchoolName }).ToList();
